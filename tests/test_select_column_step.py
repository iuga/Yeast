...
def test_select_columns_step_must_fail_if_column_doesnot_exist_on_df(df):
    ...
    with pytest.raises(YeastValidationError):
        ...
        step.prepare(df).bake(df)


def test_select_columns_step_must_fail_if_column_is_not_string(df):
    """
